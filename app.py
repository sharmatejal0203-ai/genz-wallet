...
.stApp {
    background:
        radial-gradient(circle at 50% -10%, #202438 0%, #0B0C10 38%, #08090C 100%);
    color: #F5F5F7;
}
...
.balance-card {
    background:
        linear-gradient(145deg, #191C25, #0F1116);
...
.intelligence {
    background:
        linear-gradient(145deg, #181520, #101117);
...
.virtual-card {
    background:
        linear-gradient(135deg, #222630, #101218);
    ...
    if st.session_state.show_request:
        ...
        if st.button(
            ...
            if not person.strip():
                ...
                reason = reason.strip() or "Payment request"
